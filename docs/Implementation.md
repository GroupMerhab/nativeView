...
- **Java / Android bindings**
  ...
  - **Android embedding docs:** **`docs/Android.md`**, **`docs/Android-migration.md`**, sample app **`examples/android_full_bridge/`**; WebView **`deliverWebMessage`** falls through to **`nativeDispatchWebMessage`** when no Java handler matches so **`app.handshake`** and other C-side ops run.
