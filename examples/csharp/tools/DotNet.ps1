# Resolve dotnet.exe with an installed SDK (skips runtime-only hosts on PATH).
...
function Resolve-DotNetExe {
    ...
    if ($env:ProgramFiles) { $roots.Add((Join-Path $env:ProgramFiles 'dotnet')) }
    if (${env:ProgramFiles(x86)}) { $roots.Add((Join-Path ${env:ProgramFiles(x86)} 'dotnet')) }
    if ($env:LOCALAPPDATA) { $roots.Add((Join-Path $env:LOCALAPPDATA 'Microsoft\dotnet')) }

    $relPaths = @('dotnet.exe', 'x64\dotnet.exe', 'arm64\dotnet.exe')
    ...
    throw @"
Could not find a .NET SDK (dotnet.exe with installed SDKs).

Install the .NET 8 SDK (or newer): https://dotnet.microsoft.com/download/dotnet/8.0

If the SDK is installed but you see "No .NET SDKs were found", Windows often has a
runtime-only dotnet under Program Files (x86) that shadows the real SDK. Either:
  - Remove C:\Program Files (x86)\dotnet from PATH, or put C:\Program Files\dotnet first, or
  - Set DOTNET_EXE to the SDK host, for example:
