...
watch(
  () => todos.archivedTodos.map((t) => t.id).join(','),
  () => {
    const allowed = new Set(todos.archivedTodos.map((t) => t.id))
    const next = new Set()
...
const rows = computed(() => todos.archivedTodos)
...
function onRestore() {
  ...
  todos.restoreArchivedIds([...selected.value])
  selected.value = new Set()
}
</script>
...
<template>
  <section class="nv-card">
    ...
    <p class="hint">Archived tasks are hidden from active lists. Restore moves them back to Done.</p>

    <p v-if="!rows.length" class="empty">No archived todos yet.</p>

    <div v-else class="wrap">
      <table class="tbl" aria-label="Archived todos">
        <thead>
          <tr>
            <th class="col-check" scope="col">
              <input
                type="checkbox"
                :checked="allSelected"
                aria-label="Select all archived todos"
